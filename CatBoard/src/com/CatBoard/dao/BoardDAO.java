...
public class BoardDAO {
	...
	public void boardUpdate(BoardVO board) {
		...
		try {
			conn = connect();
			pstmt = conn.prepareStatement("update board set title=?,content=?,MODIFY_TIME=? where board_num=?");
			pstmt.setString(1, board.getTitle());
			pstmt.setString(2, board.getContent());
			pstmt.setString(3, board.getMODIFY_TIME());
			pstmt.setString(4, board.getBoard_num());
			pstmt.executeUpdate();
	...
	// 글 삭제
	public void boardDelete(String board_num) { // id 받아오기 근데 이거 board_num이여야 하지 않을까..
		Connection conn = null;
		PreparedStatement pstmt = null;
	...
	public ArrayList<BoardVO> boardList(String category) {
		...
		try {
			conn = connect();
			pstmt = conn.prepareStatement("select board_num,board_id,id,title,CREATE_TIME from board where board_id = ?");
			pstmt.setString(1, category);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				board = new BoardVO();
				board.setBoard_num(rs.getString(1)); //board_num
	...
	public BoardVO boardArticle(String num) {
		...
		try {
			conn = connect();
			pstmt = conn.prepareStatement("select board_num, board_id, id, title, content, CREATE_TIME from board where board_num=?");
			pstmt.setString(1, num);
			rs = pstmt.executeQuery();
			
			if (rs.next()) {
				board = new BoardVO();
				board.setBoard_num(rs.getString(1));
